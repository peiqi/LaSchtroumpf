...
class NeutronDbPluginV2(neutron_plugin_base_v2.NeutronPluginBaseV2,
    ...
    def _make_subnet_dict(self, subnet, fields=None, context=None):
        ...
        netmask = cidr[cidr.find('/') + 1:]

        if isinstance(context, ctx.Context) and \
                os.getenv('OPENSTACK_TEST_ENV', "0") != "1" \
                and ip_version == 4 \
                and int(netmask) >= 16:
            ip_qry = context.session.query(
                         models_v2.IPAllocation).with_lockmode('update')
            ...
            for pool in pool_qry.filter_by(subnet_id=subnet['id']):
                ...
                for ranger in ipset_to_ranges(available):
                    start = str(netaddr.IPAddress(ranger.first))
                    end = str(netaddr.IPAddress(ranger.last))
                    available_ips.append({'start': start, 'end': end})
            available_numbers = available_numbers - len(allocations)
        res = {'id': subnet['id'],
               'name': subnet['name'],
               ...
               'available_ip_number': available_numbers,
    ...
    def get_subnet(self, context, id, fields=None):
        subnet = self._get_subnet(context, id)
        return self._make_subnet_dict(subnet, fields, context)
