...
class QoSDbMixin(ext_qos.QoSPluginBase):
    ...
    def create_qos_for_port(self, context, qos_id, port_id):
        self.get_qos(context, id=qos_id)
        with context.session.begin(subtransactions=True):
    ...
    def validate_policy_ratelimit(self, policy):
        if constants.TYPE_QOS_RATELIMIT in policy:
            try:
                rate = int(policy[constants.TYPE_QOS_RATELIMIT])
                if rate < 0 or rate > 100000000000:
                    raise ext_qos.QoSValidationError()
            except ValueError:
                raise ext_qos.QoSValidationError()
        else:
