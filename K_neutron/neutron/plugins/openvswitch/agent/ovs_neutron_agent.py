...
class OVSQoSAgent(qos_rpc.QoSAgentRpcMixin):
    ...
    def sync_qos(self, cur_ports):
        '''sync qos.

        :param cur_ports: ports
        '''
        ports_dict = {}
        for port in cur_ports:
            if port.port_name[:3] != "qvo":
                continue

            police = self.plugin_rpc.get_port_and_qos_policy(self.context,
                                                             port.vif_id)
            if police:
                ports_dict.update(police)
            else:
                ports_dict[port.vif_id] = None
        self.port_qos_sync(self.context, ports_dict)
    ...
    def init_qos(self):
        ...
        if 'OpenflowQoSVlanDriver' in cfg.CONF.qos.qos_driver:
            ...
            if self.phys_brs:
                ...
                self.qos_agent.init_qos(bridge=self.int_br,
                                        #ext_bridge=external_bridge,
        ...
        elif 'OVSHybridQoSDriver' in cfg.CONF.qos.qos_driver:
            # TODO(scollins) - Make this configurable, if there is
            # more than one physical bridge added to
            # bridge_mappings
            if self.int_br:
