...
from neutron.agent import ha_agent
...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def create_port(self, context, port):
        ...
        try:
            self._set_default_qos(context, result)
        except Exception:
            LOG.error(_LE("port %s create default qos failed"), result['id'])
    ...
    def delete_port(self, context, id, l3_port_check=True):
        ...
        with contextlib.nested(lockutils.lock('db-access'),
            ...
            try:
                self._delete_default_qos(context, port)
            except Exception:
                LOG.error(_LE("port %s delete default qos failed"), id)
