...
# modified by bc-vnetwork, bug: BCVSW-149
from neutron.services.qos.drivers.floating_ip_linux_tc import \
    FloatingIpLinuxTcQoSDriver
...
# modified by bc-vnetwork, bug: BCVSW-149
class FloatingIpQoSAgent(qos_rpc.QoSAgentRpcMixin):

    def __init__(self, context, driver, plugin_rpc, conf, **kwargs):
    ...
    def init_qos(self, *args, **kwargs):
        '''Init qos.'''
        if cfg.CONF.qos.floating_ip_qos_driver:
            if 'LinuxTCFloatingIPQoSDriver' == \
                    cfg.CONF.qos.floating_ip_qos_driver:
                self.root_helper = config.get_root_helper(self.conf)
                self.qos = FloatingIpLinuxTcQoSDriver(self.root_helper)
                return True
        LOG.debug(_('No QoS Driver is loaded for Floating IP ports'))
        return False

    def get_external_device_name(self, port_id):
        '''Get external device name
    ...
    def update_routers(self, routers):
        ...
        for r in routers:
            router_id = r['id']
            floating_ips = r.get('_floatingips', None)
            gw_port = r.get('gw_port', None)
            if gw_port is None:
                continue
            gw_port_id = gw_port.get('id', None)
            if gw_port_id is None:
                continue
            gw_port_id = self.get_external_device_name(gw_port_id)
            if floating_ips is None:
                continue
            ns_name = NS_PREFIX + router_id
            for _floating_ip in floating_ips:
                if _floating_ip is not None:
                    floating_port_id = _floating_ip.get('floating_port_id',
                                                        None)
                    floating_ip_address = _floating_ip.get(
                        'floating_ip_address', None)
                    if floating_ip_address is not None and \
                            floating_ip_address != '':
                        _policy = self.plugin_rpc.get_port_and_qos_policy(
                            self.context, floating_port_id)
                        if len(_policy) == 1:
                            policy = _policy.get(floating_port_id, None)
                            if policy is not None:
                                port_details = {
                                    'router_id': router_id,
                                    'gw_port_id': gw_port_id,
                                    'ns_name': ns_name,
                                    'floating_port_id': floating_port_id,
                                    'floating_ip_address': floating_ip_address,
                                    'inbound_rate': int(policy['ratelimit']),
                                    # 'inbound_latency': -1,
                                    # 'inbound_burst': -1,
                                    # 'outbound_latency': -1,
                                    # 'outbound_burst': -1,
                                    'outbound_rate': int(policy['ratelimit']),
                                }
                                if floating_port_id not in floating_ip_ports:
                                    floating_ip_ports[(ns_name,
                                                       floating_port_id)] = \
                                                       port_details
        self.qos.cb_update_floating_port(floating_ip_ports)


class QoSAgentFloatingIpRpcCallbackMixin(object):
    qos_agent = None

    def get_fip_port_details(self, port_id):
        '''Get floating ip port details.

        :param port_id: port id
        '''
        for _, ri_value in self.router_info.items():
            for fip in ri_value.router.get(l3_constants.FLOATINGIP_KEY, []):
                if port_id is None or port_id == fip.get("floating_port_id",
                                                         ""):
                    fip_port_details = {}
                    fip_port_details['ns_name'] = ri_value.ns_name
                    fip_port_details['router_id'] = ri_value.router_id
                    gw_port_id = ri_value.router.get('gw_port').get('id', '')
                    fip_port_details['gw_port_id'] = \
                        ri_value.get_external_device_name(gw_port_id)
                    fip_port_details['floating_ip_address'] = \
                        fip.get("floating_ip_address", None)
                    fip_port_details['floating_ip_id'] = fip.get("id", None)
                    fip_port_details['floating_port_id'] = \
                        fip.get("floating_port_id", None)
                    return fip_port_details
        return None
    ...
    def port_qos_deleted(self, context, **kwargs):
        ...
        :param context: context
        '''
        if not self.qos_agent:
            return
        qos_id = kwargs.get('qos_id', '')
        port_id = kwargs.get('port_id', '')
        LOG.debug(_('QoS %(qos_id)s updated on floating_ip_port: %(port_id)s')
                  % kwargs)
        fip_context = self.get_fip_port_details(port_id)
        if fip_context:
            self.qos_agent.port_qos_deleted(context, qos_id,
                                            port_id, ctx=fip_context)
        else:
            LOG.error(_('QoS %(qos_id)s updated on floating_ip_port: '
                        '%(port_id)s'
                        ' failed, no floating_ip_port is found on the host')
                      % kwargs)
    ...
    def port_qos_updated(self, context, **kwargs):
        ...
        fip_context = self.get_fip_port_details(port_id)
        if fip_context:
            self.qos_agent.port_qos_updated(context, qos_id,
                                            port_id, ctx=fip_context)
        else:
            LOG.error(_('QoS %(qos_id)s updated on floating_ip_port: '
                        '%(port_id)s'
...
# modified by bc-vnetwork, bug: BCVSW-149
class L3PluginApi(qos_rpc.QoSServerRpcApiMixin):
...
class L3NATAgent(firewall_l3_agent.FWaaSL3AgentRpcCallback,
                 ...
                 manager.Manager,
                 # modified by bc-vnetwork, bug: BCVSW-149
                 QoSAgentFloatingIpRpcCallbackMixin):
    ...
    def setup_rpc(self):
        ...
        consumers = [[topics.QOS, topics.UPDATE]]
    ...
    def init_qos(self):
        '''Init QoS agent'''
        self.qos_agent = FloatingIpQoSAgent(self.context,
                                            self.driver,
                                            self.plugin_rpc,
                                            self.conf)
        if self.qos_agent.init_qos():
            self.setup_rpc()
        else:
            self.qos_agent = None
    ...
    def __init__(self, host, conf=None):
        ...
        self.init_qos()  # modified by bc-vnetwork, bug: BCVSW-149
    ...
    def fetch_and_sync_all_routers(self, context, ns_manager):
        ...
        try:
            if self.conf.use_namespaces:
                ...
                routers = self.plugin_rpc.get_routers(context,
                                                      [self.conf.router_id])
            # update floating ip ports for qos, bug: BCVSW-149
            if self.qos_agent:
                self.qos_agent.update_routers(routers)
