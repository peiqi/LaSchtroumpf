...
class L3HATestCase(L3HATestFramework):
    ...
    def test_sync_ha_router_info_without_ha_port(self):
        router1 = self._create_router()
        self._bind_router(router1['id'])
        router2 = self._create_router()
        self._bind_router(router2['id'])

        # only retrieve the router1 ha port bindings
        bindings = self.plugin.get_ha_router_port_bindings(
            self.admin_ctx, [router1['id']])
        for binding in bindings:
            self.assertIsNotNone(binding.port)

        # To simulate the race, so here delete ha port of router1
        # then the ha_port_bindings is removed too
        for binding in bindings:
            self.core_plugin.delete_port(self.admin_ctx, binding.port['id'])

        routers = self.plugin.get_ha_sync_data_for_host(self.admin_ctx,
                                                        self.agent1['host'],
                                                        self.agent1)

        # the l3 agent will never got the router without HA interfaces
        # so here wil only got the HA router2
        for router in routers:
            self.assertIsNotNone(router.get(constants.HA_ROUTER_STATE_KEY))
